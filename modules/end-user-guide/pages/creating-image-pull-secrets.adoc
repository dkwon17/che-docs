...
To allow the `{devworkspace}` Pods to access container registries requiring authentication, create an image pull Secret by using `{orch-cli}` or a `.dockercfg` file or a `config.json` file.
