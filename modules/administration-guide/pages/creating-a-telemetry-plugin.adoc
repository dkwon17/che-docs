...
* Creating a `plugin.yaml` file representing a {devworkspace} plugin for the custom backend.
* Specifying of a location of a custom plugin to {prod-short} by setting the `workspacesDefaultPlugins` attribute from the `CheCluster` custom resource.
...
. Adding a plugin for your backend and instructing {prod-short} to load the plugin in your {devworkspace}s
...
NOTE: For fast feedback when developing, it is recommended to do development inside a {devworkspace}. This way, you can run the application in a cluster and receive events from the front-end telemetry plugin.
...
. To verify that the `onEvent()` method of `AnalyticsManager` receives events from the front-end plugin, press the kbd:[l] key to disable Quarkus live coding and edit any file within the IDE. The following output should be logged:
+
----
...
Running `mvn quarkus:dev` as described in xref:running-the-application[] and terminating the application with kbd:[Ctrl+C] sends a `WORKSPACE_STOPPED` event to the server.
...
[source,yaml]
----
include::example$creating-a-telemetry-plugin/Dockerfile.jvm[]
----
====
...
[source,yaml]
----
include::example$creating-a-telemetry-plugin/Dockerfile.native[]
----
====
To build the image, run:
...
[source,yaml]
----
include::example$creating-a-telemetry-plugin/plugin.yaml[]
----
====
...
----
include::example$creating-a-telemetry-plugin/webserver.yaml[]
----
====
...
[id="specifying-the-telemetry-plugin-in-a-devworkspace"]
...
== Applying the telemetry plugin for all {devworkspace}s
...
spec:
  devEnvironments:
    defaultPlugins:
    - editor: che-incubator/che-code/insiders     <1>
      plugins:                           <2>
...
. Verify that the telemetry plugin is working by following the verification steps for xref:specifying-the-telemetry-plugin-in-a-devworkspace[].
