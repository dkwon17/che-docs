...
. Create a ClusterRoleBinding to bind the `prometheus-k8s` ServiceAccount to the link:https://github.com/devfile/devworkspace-operator/blob/main/deploy/deployment/kubernetes/objects/devworkspace-controller-metrics-reader.ClusterRole.yaml[devworkspace-controller-metrics-reader] ClusterRole. For the xref:configuring-prometheus-and-grafana-using-templates.adoc[example monitoring stack], the name of the ServiceAccount to be be used is `prometheus`.
+
.ClusterRoleBinding
====
