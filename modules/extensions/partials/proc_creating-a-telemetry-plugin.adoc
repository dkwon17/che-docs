...
. For the purposes of this example, remove the files under `src/test/java/mygroup`.
...
. From a terminal window, run the application by running the following command within a {devworkspace}. Use the `--settings` flag to specify the location of the `settings.xml` file that contains the GitHub access token.
+
----
...
+
image::telemetry/devworkspace_telemetry_sample_plugin.png[]
