...
A finished example of the telemetry back-end is available link:https://github.com/che-incubator/devworkspace-telemetry-example-plugin[here].
...
. From a terminal window, run the application by running the following command within a {devworkspace}. Use the `--settings` flag to specify path to the location of the `settings.xml` file that contains the GitHub access token.
+
----
...
For the following example, the telemetry server application is deployed to OpenShift at the following URL: `++http://little-telemetry-server-che.apps-crc.testing++`, where `apps-crc.testing` is the ingress domain name of the OpensShift cluster.
...
Running `mvn quarkus:dev` as described in xref:running-the-application_{context}[] and terminating the application with kbd:[Ctrl+C] sends `WORKSPACE_STOPPED` event sent to the server.
...
[source,yaml]
----
include::example$telemetry/plugin.yaml[]
----
====
...
+
image::telemetry/devworkspace_telemetry_plugin.png[]

. Edit files within the editor and observe their events in the example telemetry server logs.
